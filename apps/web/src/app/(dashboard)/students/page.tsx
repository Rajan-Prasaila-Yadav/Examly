...
export default function StudentsPage() {
  ...
  const handleConfirmAssignBatch = async () => {
    if (!assigningStudent || !targetBatchId) return;
    try {
      setIsAssigning(true);
      await api.put(`/users/students/${assigningStudent.id}/batch`, { batchId: targetBatchId });
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {students.map((stu) => {
                const currentBatchName =
                  stu.studentProfile?.batch?.name ||
                  batches.find((b: any) => b.id === stu.studentProfile?.batchId)?.name ||
                  'CEE 2026 Medical Entrance Batch';
                  ...
                  <tr key={stu.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-3.5 px-4">
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-brand-50 hover:bg-brand-100 text-brand-700 text-[11px] font-bold border border-brand-200/80 transition-colors group cursor-pointer"
                      >
                        <GraduationCap className="w-3 h-3 text-brand-600 group-hover:scale-110 transition-transform" />
                        <span>{currentBatchName}</span>
                        <span className="text-[9px] bg-brand-200/60 px-1 py-0.2 rounded font-normal ml-0.5 text-brand-900">
                          Edit ▾
                        </span>
                      </button>
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          onClick={() => handleOpenAssignBatch(stu)}
                          className="px-2.5 py-1 bg-brand-50 hover:bg-brand-100 text-brand-700 text-[11px] font-bold rounded-lg transition-colors border border-brand-200/60 flex items-center gap-1"
                          title="Assign Student to Active Batch"
                        >
                          <Layers className="w-3 h-3 text-brand-600" /> Assign Batch
                        </button>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl border border-slate-200 p-6 space-y-5">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-900">Assign Student to Batch</h3>
